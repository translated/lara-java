...
public class AudioTranslator {
    ...
    public Audio upload(
            ...
            String source,
            String target,
            AudioUploadOptions options
    ) throws LaraException, S3Exception {
        S3UploadParams s3Params = uploadToS3(file, file.getName());
    ...
    /**
     * Uploads an audio file and creates a translated transcript request for the target language.
     *
     * @param file   input audio file
     * @param target ISO language code of the target language
     * @return an {@link Audio} object representing the created transcript request
     * @throws LaraException if an error occurs on the LARA API side
     * @throws S3Exception   if an error occurs while uploading to S3
     */
    public Audio uploadForTranscription(
            File file,
            String target
    ) throws LaraException, S3Exception {
        return uploadForTranscription(
                file,
                file.getName(),
                null,
                target,
    ...
    /**
     * Uploads an audio file and creates a translated transcript request.
     *
     * @param file   input audio file
     * @param source ISO language code of the source language; may be {@code null} for auto-detection
     * @param target ISO language code of the target language
    ...
    /**
     * Uploads an audio file and creates a translated transcript request with advanced options.
     *
     * @param file    input audio file
     * @param source  ISO language code of the source language; may be {@code null} for auto-detection
     * @param target  ISO language code of the target language
     * @param options transcript translation options; may be {@code null}
     * @return an {@link Audio} object representing the created transcript request
     * @throws LaraException if an error occurs on the LARA API side
     * @throws S3Exception   if an error occurs while uploading to S3
     */
    public Audio uploadForTranscription(
            ...
            String source,
            String target,
            AudioTranscriptOptions options
    ) throws LaraException, S3Exception {
        return uploadForTranscription(
                file,
                file.getName(),
    ...
    /**
     * Uploads an audio file using an explicit filename and creates a translated transcript request.
     *
     * @param file     input audio file
     * @param filename filename to send to the upload-url endpoint
     * @param source   ISO language code of the source language; may be {@code null} for auto-detection
     * @param target   ISO language code of the target language
     * @param options  transcript translation options; may be {@code null}
     * @return an {@link Audio} object representing the created transcript request
    ...
    public Audio uploadForTranscription(
        ...
        S3UploadParams s3Params = uploadToS3(file, filename);

        HttpParams<Object> params = (options == null ? new HttpParams<>() : options.toParams())
                .set("target", target)
                .set("source", source)
                .set("s3key", s3Params.getS3Key());

        return client.post(
                "/v2/audio/translate-transcript",
                params.build(),
                null,
                transcriptNoTraceHeaders(options)
        ).as(Audio.class);
    }
    ...
    /**
     * Retrieves the translated transcript JSON for an already translated audio job.
     *
     * @param id identifier of the audio resource returned during upload or translation
     * @return structured translated transcript result
     * @throws LaraException if an error occurs on the LARA API side
     */
    public AudioTextResult getTranslatedTranscript(String id) throws LaraException {
        String path = "/v2/audio/" + id + "/translated-transcript";
        return client.get(path).as(AudioTextResult.class);
    }
    ...
    public InputStream translate(
            File input,
            String target
    ) throws LaraException, S3Exception, InterruptedException {
    ...
    /**
     * Performs a synchronous transcript translation of an audio file.
     *
     * @param input  input audio file
     * @param target ISO language code of the target language
     * @return structured translated transcript result
     * @throws LaraException        if an error occurs on the LARA API side
     * @throws S3Exception          if an error occurs during upload on S3
     * @throws InterruptedException if the thread is interrupted while polling
     */
    public AudioTextResult translateTranscript(
            File input,
            String target
    ) throws LaraException, S3Exception, InterruptedException {
        return translateTranscript(input, null, target, null);
    }

    /**
     * Performs a synchronous transcript translation of an audio file specifying source and target languages.
     *
     * @param input  input audio file
     * @param source ISO language code of the source language; may be {@code null} for auto-detection
     * @param target ISO language code of the target language
    ...
    public AudioTextResult translateTranscript(
        ...
        return translateTranscript(input, source, target, null);
    }

    /**
     * Performs a synchronous transcript translation of an audio file with advanced options.
     *
     * @param input   input audio file
     * @param source  ISO language code of the source language; may be {@code null} for auto-detection
     * @param target  ISO language code of the target language
    ...
    /**
     * Performs a synchronous transcript translation of an audio file using
     * the specified filename for the upload request.
     *
     * @param input    input audio file
     * @param filename filename to use when uploading the audio
     * @param source   ISO language code of the source language; may be {@code null} for auto-detection
     * @param target   ISO language code of the target language
     * @param options  transcript translation options; may be {@code null}
     * @return structured translated transcript result
    ...
    public AudioTextResult translateTranscript(
        ...
        Audio audio = uploadForTranscription(
                input,
                filename,
                source,
                target,
        ...
        audio = pollAudioUntilCompleted(audio);
        return getTranslatedTranscript(audio.getId());
    }

    /**
     * Retrieves the presigned upload URL and uploads the audio file to S3.
     *
     * @param file     audio file to upload
     * @param filename filename to send to the upload-url endpoint
     * @return the S3 upload parameters, including the uploaded object key
     * @throws LaraException if an error occurs while retrieving the upload URL
     * @throws S3Exception   if an error occurs while uploading the file to S3
     */
    private S3UploadParams uploadToS3(
            File file,
            String filename
    ) throws LaraException, S3Exception {
        S3UploadParams s3Params = client.get(
                "/v2/audio/upload-url?" + new HttpParams<String>()
    ...
    private Map<String, String> transcriptNoTraceHeaders(
            AudioTranscriptOptions options
    ) {
        Map<String, String> headers = new HashMap<>();
     ...
     */
    private Audio pollAudioUntilCompleted(
            Audio audio
    ) throws InterruptedException, LaraException {
        int pollingInterval = 2000;
        int maxWaitTime = 1000 * 60 * 15;
